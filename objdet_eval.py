...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            gt_x_min = min(gt_bbox[:,0])
            gt_x_max = max(gt_bbox[:,0])
            gt_y_min = min(gt_bbox[:,1])
            gt_y_max = max(gt_bbox[:,1])
            gt_area = (gt_x_max - gt_x_min) * (gt_y_max - gt_y_min)
            ## step 2 : loop over all detected objects
            for det in detections:
                ## step 3 : extract the four corners of the current detection
                pred_bbox  = tools.compute_box_corners(det[1] , det[2] , det[5] , det[6] , det[7])
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                pred_bbox = np.array(pred_bbox)
                pred_x_min = min(pred_bbox[:,0])
                pred_x_max = max(pred_bbox[:,0])
                pred_y_min = min(pred_bbox[:,1])
                pred_y_max = max(pred_bbox[:,1])
                ...
                x_inter_min = max(gt_x_min , pred_x_min)
                x_inter_max = min(gt_x_max , pred_x_max)
                inter_y_min = max(gt_y_min , pred_y_min)
                inter_y_max = min(gt_y_max , pred_y_max)
                inter_ar = max(0 , x_inter_max - x_inter_min) * max(0, inter_y_max - inter_y_min)
                union_area = gt_area + pred_area - inter_ar
                iou = inter_ar / union_area if union_area > 0 else 0
                print('Intersection Area' , inter_ar)
    ...
    false_positives = len([d for d in detections if d[0] == 1]) - true_positives
...
def cal_corn(label_arr):
    c_x , c_y , c_z , h , w , l , hd = label_arr
    ...
    r_c = np.dot(local_corners , rotation_matrix.T)
    return(r_c + np.array([c_x , c_y , c_z]) , h*w*l)
    
# evaluate object detection performance based on all frames
def compute_performance_stats(det_performance_all, config_det):
