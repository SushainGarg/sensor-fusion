...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        if 'darknet' in configs.arch:
            ...
            print("Shape of outputs['hm_cen']:", outputs['hm_cen'].shape)
    ...
    if len(detections) > 0:
        ...
        for d in detections:
            ...
            det = np.copy(d)
            _ , _x , _y , _z , _ , _w , _l , _yaw  = det
            det[1] = (_y / configs.bev_height) * (configs.lim_x[1] - configs.lim_x[0]) + configs.lim_x[0]
            det[2] = (_x / configs.bev_width) * (configs.lim_y[1] - configs.lim_y[0]) + configs.lim_y[0]
            det[3] = _z + configs.lim_z[0]
            det[5]= ( _w / configs.bev_width) * (configs.lim_y[1] - configs.lim_y[0])
            det[6] = (_l / configs.bev_height) * (configs.lim_x[1] - configs.lim_x[0])
            det[7] = -_yaw
            det[0] = 1
